...
class Command(BaseCommand):
    ...
    def s2m_get_invoicelist(self, location_id, profile_key):
    #NB: rowsleft doesn't work here, API doesn't show rows but a count of all items. 
        ...
        results=[]
        while itemsleft > -20:
            url = 'https://apiv2.seats2meet.com/api/invoices/location/%s' % location_id
    ...
    def s2m_send_invoice(self, invoiceid, fromemail, toemail, subject, mailtext, invoice_code, LocationId, profile_key):
    ...
    def get_s2m_address(self, location_id, profile_key):
        url = 'http://apiv2.seats2meet.com/api/locations/%s' % location_id
            ...
            for item in self.s2m_get_invoicelist(location.location_id, location.profile_key):
                ...
                if item.get('NoTax') == True and round(taxes,0) == round(item.get('AmountOpen'),0):
                    ...
                    if datetime.datetime.today() > two_weeks_after and datetime.datetime.today() < four_weeks_after and invoice_reminder != 1:
                        ...
                        if test_mode != 'yes':
                            try:
                                self.s2m_send_invoice(item.get('Id'), location_address.get('Email'), item.get('EmailTo'), subject, mailtext, item.get('Code'), location_address.get('Id'), location.profile_key)
                            except: #not, inform admin
                                print 'niet gelukt'
                                #email admin about failure
                                send_mail('Failed to send reminder for invoice %s' % item.get('Code'), 'Wasnt able to send the reminder for the invoice. perhaps the email address is missing in the account? Please send manually.', 'felix@donfelicio.com',[location_address.get('Email')], fail_silently=False)
                            else: #done
                                print 'gelukt'
                    ...
                    if datetime.datetime.today() >= six_weeks_after and datetime.datetime.today() <= eight_weeks_after and invoice_reminder != 3:
                        ...
                        if test_mode != 'yes':                        
                            try:
                                self.s2m_send_invoice(item.get('Id'), location_address.get('Email'), item.get('EmailTo'), subject, mailtext, item.get('Code'), location_address.get('Id'), location.profile_key)
                                send_mail('Just sent third reminder for invoice %s' % item.get('Code'), 'Perhaps its time to call your customer, they should really pay their invoice.', 'felix@donfelicio.com',[location_address.get('Email')], fail_silently=False)
                            except: #not, inform admin
                                print 'niet gelukt'
                                #email admin about failure
                                send_mail('Failed to send reminder for invoice %s' % item.get('Code'), 'Wasnt able to send the reminder for the invoice. perhaps the email address is missing in the account? Please send manually.', 'felix@donfelicio.com', [location_address.get('Email')], fail_silently=False)
                    ...
                    if datetime.datetime.today() >= eight_weeks_after and invoice_reminder != 4:
                        ...
                        if test_mode != 'yes':
                            try:
                                self.s2m_send_invoice(item.get('Id'), location_address.get('Email'), item.get('EmailTo'), subject, mailtext, item.get('Code'), location_address.get('Id'), location.profile_key)
                                send_mail('Just sent FOURTH and final reminder for invoice %s' % item.get('Code'), 'Please call your customer, they should pay the invoice today!', 'felix@donfelicio.com',[location_address.get('Email')], fail_silently=False)
                            except: #not, inform admin
                                print 'niet gelukt'
                                #email admin about failure
                                send_mail('Failed to send reminder for invoice %s' % item.get('Code'), 'Wasnt able to send the reminder for the invoice. perhaps the email address is missing in the account? Please send manually.', 'felix@donfelicio.com', [location_address.get('Email')], fail_silently=False)
